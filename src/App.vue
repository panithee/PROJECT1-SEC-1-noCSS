...
<template>
  <div class="w-screen h-screen">
    <div class="absolute w-full h-full bg-bgPage">
      ...
      <div
        class="w-5/6 m-auto mt-10 overflow-y-scroll rounded-3xl bg-bgBox sm:w-1/2 h-1/2"
        v-if="sw"
      >
          ...
          <tbody>
            <tr v-for="(food, index) in foodLists" key="index">
              <td class="pl-10 text-2xl text-left">
                ...
                >
                  <div
                    v-for="(person, index) in foodLists[index].person"
                    key="index"
                  >
                    <span
                      class="px-1 ml-1 text-base"
                      :class="['color-' + (index % 4)]"
                      >{{ person.name }}</span
                    >
                  </div>
      ...
      >
        <button
          class="w-full h-full text-xl text-white"
          @click="eventFoodList($event, 'add')"
        >
          ADD
        </button>
      </div>
        ...
        <table class="w-full text-brownFont">
          ...
          <tbody>
            <tr
              ...
              key="index"
              :class="person.status ? 'bg-checkboxGreen' : 'bg-bgBox'"
            >
    ...
    <div class="fixed w-full h-full" v-show="showMenuStatus">
      ...
      >
        <div class="lg"></div>
        <div
          class="flex flex-col w-[336px] h-[304px] sm:w-[600px] sm:h-[400px] md:w-[776px] md:h-[432px] bg-bgPage rounded-[40px]"
        >
          ...
          <div class="flex">
            ...
            <div class="flex px-[56px]">
              <div
                class="flex flex-col items-center rounded-full bg-bgList1 lg:w-[304px] lg:h-[304px]"
              >
                <div
                  class="flex justify-center items-center h-[168px] w-[296px] rounded-t-full bg-bgList1"
                >
                  <input
                    type="number"
            ...
            <div class="flex flex-col">
              <div class="flex flex-col items-start">
                ...
                <input
                  ...
                  v-model="foodName"
                  class="mt-1 bg-bgbtn w-[296px] h-[48px] rounded-[20px] pl-4 text-[24px] text-brownFont"
                  type="text"
                />
                ...
                <div
                  :class="
                    personFood.length !== 0
                      ? 'flex mt-2 bg-bgbtn w-[280px] h-[92px] rounded-2xl'
                      : 'flex mt-2 bg-bgbtn w-[280px] h-[92px] rounded-2xl justify-center items-center'
                  "
                >
                  <div
                    v-if="personFood.length == 0"
                    class="flex items-center justify-center px-2 m-2 h-9 text-slate-700"
                  >
                    No one right now~~
                  </div>
                  <div
                ...
                <div class="flex flex-row mt-4">
                  ...
                  <button
                    class="ml-5 text-xl underline text-btn1"
                    @click="deleteBtn"
                  >
                    Delete
                  </button>
